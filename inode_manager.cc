   ...
   */
    ...
    if(size >= size_old)
    {
        if(block_num_new > NDIRECT)
        {
            ...
            {
                bm -> read_block(inode -> blocks[NDIRECT], indirect);
                start = block_num_old - NDIRECT;
            }
            for(unsigned int j = start; j < block_num_new - NDIRECT; j++)
            {
                *((blockid_t *)indirect + j) = bm -> alloc_block();
            }
    ...
    else
    {
        ...
        else
        {
            ...
            for(unsigned int j = block_num_new; j < MIN(block_num_old, NDIRECT); j++)
            {
